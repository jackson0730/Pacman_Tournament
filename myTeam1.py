...
class TestHeuristicAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #reduce enemy ghost scared time
        # if self.timer > 0:
        #     self.timer -= 1
        ...
        if foodLeft < 3:
            ...
            for action in actions:
                ...
                pos2 = successor.getAgentState(self.index)
                dist = self.getMazeDistance(self.start,pos2)
                if dist < bestDist:
                    bestAction = action
                    bestDist = dist
            return bestAction
        #print('best actions are:', bestActions)
        ...
        #
        # if self.foodCollected >= 10:
        #     choice = self.backHome(gameState)

        return choice

    def backHome(self, gameState):
        dist = float('inf')
        bestAction = Directions.REVERSE

        for i in gameState.getLegalActions(self.index):
            succ = self.getSuccessor(gameState, i)
            if self.getClosestEnemyPosition(succ) < 5:
                continue
            nextPosition = succ.getAgentState(self.index).getPosition()
            if self.getMazeDistance(nextPosition,self.start) < dist:
                dist = self.getMazeDistance(nextPosition,self.start)
                bestAction = i

        if not self.getSuccessor(gameState, bestAction).getAgentState(self.index).isPacman:
            self.foodCollected = 0
    ...
    def getClosestEnemyPosition(self, gameState):
        ...
        for i in self.getOpponents(gameState):
            if gameState.getAgentPosition(i) != None:
                enemiesList.append((gameState.getAgentState(i), gameState.getAgentPosition(i)))
        for enemy, enemyPos in enemiesList:
...
class TestHeuristicAgentO(TestHeuristicAgent):
    ...
    def evaluate(self, gameState, action):
        ...
        defenders = [a for a in enemies if
                     not (a.isPacman) and a.scaredTimer <= 0 and a.getPosition() != None and self.getMazeDistance(myPos,
                                                                                                                a.getPosition()) <= 6]
        if len(defenders) > 0:
            score += -3000
            ...
            # safeCapsule = [self.getMazeDistance(myPos, capsule) for capsule in capsuleList if self.hutongMap.value(capsule) == 0]
            # score += -300 * len(safeCapsule)
            # if not safeCapsule == []:
            #     minSafeCapsuleDistance = min(safeCapsule)
            #     score += -2 * minSafeCapsuleDistance
            if capsuleList != []:
...
class TestHeuristicAgentD(TestHeuristicAgent):
    def evaluate(self, gameState, action):
        ...
        #print("defensiveAgentPositionList:", dPosList)
        if myState.isPacman:
            score += -200
        enemies = [successor.getAgentState(i) for i in self.getOpponents(successor)]
